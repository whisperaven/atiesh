...
trait KafkaSourceSemantics
  ...
  /**
   * KafkaSource does not check the ack before commit, do it yourself by
   * override this method, and check it before invoke it.
   */
  override def doneCycle(ack: Acknowledge): Unit =

...
trait SinkSemantics
    ...
    override def receive: Receive = LoggingReceive {
      case event: Event =>
        ...
        try {
          ...
          case exc: Throwable =>
            logger.error(s"sink <${getName}> throw unexpected exception " +
                         s"inside handler process (single event), whitch " +
                         s"means you may use a sink component with " +
      ...
      case Batch(events, aggregatedBy) =>
        ...
        try {
          ...
          case exc: Throwable =>
            logger.error(s"sink <${getName}> throw unexpected exception " +
                         s"inside handler process (event batch with tag <" +
                         s"${aggregatedBy}>), whitch means you may use a " +
                         s"sink component with wrong implementation", exc)
        }
        ...
        try {
          ...
          case exc: Throwable =>
            logger.error(s"sink <${getName}> throw unexpected exception " +
                         ...
                         s"whitch means you may use a sink component " +
                         s"with wrong implementation", exc)
        }

      case Commit(committer, tran) =>
        ack(committer, tran)
  ...
  final private[sink] var ref: ActorRef = _
  ...
  final def commit(committer: String,
                   tran: Promise[Transaction]): Future[Transaction] = {
    logger.debug("sink <{}> triggering commit transaction statement <{}@{}>",
                 getName, tran, tran.hashCode.toHexString)
    ref ! Commit(committer, tran)
    tran.future
  }
  ...
  def ack(committer: String, tran: Promise[Transaction]): Unit = {
    logger.debug(

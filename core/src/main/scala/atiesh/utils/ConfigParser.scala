...
import com.typesafe.config.{ Config, ConfigFactory,
                             ConfigParseOptions, ConfigSyntax }

object ConfigParser {
  def parseConfigFile(cfp: File): Try[Configuration] = Try {
    if (!cfp.exists() || !cfp.isFile()) {
      throw new ConfigParseException(
        s"configuration file ${cfp.getAbsolutePath()} " +
        s"does not exists or not a file")
    } else if (!cfp.canRead()) {
      throw new ConfigParseException(
        s"can not open file ${cfp.getAbsolutePath()} " +
        s"for read (maybe permission deny)")
    } else {
